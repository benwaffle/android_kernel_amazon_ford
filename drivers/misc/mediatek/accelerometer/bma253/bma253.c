...
extern struct acc_hw* get_bma253_cust_acc_hw(void);
...
static struct i2c_board_info __initdata bma_i2c_info = {
	I2C_BOARD_INFO(BMA_DEV_NAME, 0x18)
};
...
int bma2xx_operate(void *self, uint32_t command, void *buff_in, int size_in,
		...
		const struct i2c_device_id *id)
{
	...
	sobj.self = obj;
	sobj.polling = 1;
	sobj.sensor_operate = bma2xx_operate;
...
static int bma2xx_local_init(void) 
{
	struct acc_hw *hw = get_bma253_cust_acc_hw();
	GSE_FUN();
