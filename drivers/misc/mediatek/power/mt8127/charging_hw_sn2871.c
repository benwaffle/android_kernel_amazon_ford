...
static kal_uint32 charging_hw_init(void *data)
{
	...
	sn2871_reg_config_interface(sn2871_CON2, 0x11);    
	sn2871_reg_config_interface(sn2871_CON3, 0xa);
	sn2871_reg_config_interface(sn2871_CON4, 0x20);
	sn2871_reg_config_interface(sn2871_CON9, 0x48);

	/*PreCC mode */
	sn2871_config_interface(sn2871_CON5, 0x1, 0xF, 4);
	sn2871_config_interface(sn2871_CON6, 0x1, 0x1, 1);
	/*CV mode */
	sn2871_config_interface(sn2871_CON6, 0x0, 0x1, 0);
	sn2871_config_interface(sn2871_CON7, 0x1, 0x1, 7);
	sn2871_set_FORCE_VINDPM(1);
	return status;
}
...
#endif
	battery_xlog_printk(BAT_LOG_CRTI, "charging_enable is :%d\n",enable);
        if (KAL_TRUE == enable) {
                /* sn2871_config_interface(sn2871_CON3, 0x1, 0x1, 4); //enable charging */
                sn2871_set_en_hiz(0x0);
                sn2871_chg_en(enable);
        } else {
                /* sn2871_config_interface(sn2871_CON3, 0x0, 0x1, 4); //enable charging */
                sn2871_chg_en(enable);
                if (charging_get_error_state())
                        battery_xlog_printk(BAT_LOG_CRTI, "[charging_enable] under test mode: disable charging\n");

                /*sn2871_set_en_hiz(0x1);*/
        }
...
static kal_uint32 charging_set_cv_voltage(void *data)
{
	...
	battery_xlog_printk(BAT_LOG_CRTI, "charging_set_cv_voltage register_value=0x%x %d %d\n",
	        register_value, *(unsigned int *) data, set_cv_voltage);
	sn2871_set_vreg(register_value);
...
static kal_uint32 charging_set_current(void *data)
{
	...
	battery_xlog_printk(BAT_LOG_CRTI, "charging_set_current register_value=0x%x %d %d\n",
			register_value, *(unsigned int *) data, set_chr_current);
	sn2871_set_ichg(register_value);
...
static kal_uint32 charging_set_input_current(void *data)
{
   	...
   	unsigned int status = STATUS_OK;
		...
		battery_xlog_printk(BAT_LOG_CRTI, "charging_set_input_current register_value=0x%x %d %d\n",
			register_value, *(unsigned int *) data, set_chr_current);
        sn2871_set_iinlim(register_value);
	return status;
} 	
...
static kal_uint32 charging_get_charging_status(void *data)
{
	 ...
	 unsigned int status = STATUS_OK;
        ...
        sn2871_read_interface(sn2871_CONB, &reg_value, 0x3, 3);       /* ICHG to BAT */
...
static kal_uint32 charging_reset_watch_dog_timer(void *data)
{
    ...
    //sn2871_config_interface(sn2871_CON3, 0x1, 0x1, 6);    /* reset watchdog timer */
	sn2871_wd_reset(1);
 ...
 *	   None
 */
kal_int32 chr_control_interface_sn2871(CHARGING_CTRL_CMD cmd, void *data)
{
	kal_int32 status;
