...
static u8 tpd_proximity_flag;	/*flag whether start alps*/
static u8 tpd_proximity_detect = 1;	/*0-->close ; 1--> far away*/
static struct wake_lock ps_lock;
static u8 gsl_psensor_data[8] = { 0 };

static int gsl_ps_enable;
#endif
	...
	for (source_line = 0; source_line < source_len; source_line++) {
		...
		if (0xf0 == GSL_DOWNLOAD_DATA[source_line].offset) {
			...
			if (1 ==
				...
				addr =
				    (u8) GSL_DOWNLOAD_DATA[source_line].offset;

			memcpy((buf + send_flag * 4 - 4),
			       &GSL_DOWNLOAD_DATA[source_line].val, 4);

			if (0 ==
			    send_flag % (DMA_TRANS_LEN <
					 0x20 ? DMA_TRANS_LEN : 0x20)) {
				gsl_write_interface(client, addr, buf,
						    DMA_TRANS_LEN * 4);
				send_flag = 0;
			}
...
static void check_mem_data(struct i2c_client *client)
{
	...
	print_info("[gsl1680][%s] addr = 0xb0; read_buf = %02x%02x%02x%02x\n",
		   __func__, read_buf[3], read_buf[2], read_buf[1],
		   read_buf[0]);
...
static int gsl_read_MorePage(struct i2c_client *client, u32 addr, u8 *buf,
			     u32 num)
{
	int i;
	u8 tmp_buf[4] = { 0 };
...
static int gsl_config_read_proc(struct seq_file *m, void *v)
{
	...
	if ('v' == gsl_read[0] && 's' == gsl_read[1]) {
		...
		int temp1;
		tmp =
		    (unsigned int)(((gsl_data_proc[2] << 8) | gsl_data_proc[1])
				   & 0xffff);
		buf = kzalloc(tmp, GFP_KERNEL);
		if (buf == NULL)
			return -1;
		if (3 == gsl_data_proc[0]) {
			gsl_read_interface(ddata->client, gsl_data_proc[3], buf,
					   tmp);
			if (tmp < m->size) {
			...
			temp1 =
			    ((gsl_data_proc[6] << 24) | (gsl_data_proc[5] << 16)
				 ...
				 unsigned long count, void *data)
{
...
#endif
		buf[3] =
		    char_to_int(temp_buf[14]) << 4 | char_to_int(temp_buf[15]);
		buf[2] =
		    char_to_int(temp_buf[16]) << 4 | char_to_int(temp_buf[17]);
		buf[1] =
		    char_to_int(temp_buf[18]) << 4 | char_to_int(temp_buf[19]);
		buf[0] =
		    char_to_int(temp_buf[20]) << 4 | char_to_int(temp_buf[21]);

		buf[7] =
		    char_to_int(temp_buf[5]) << 4 | char_to_int(temp_buf[6]);
		buf[6] =
		    char_to_int(temp_buf[7]) << 4 | char_to_int(temp_buf[8]);
		buf[5] =
		    char_to_int(temp_buf[9]) << 4 | char_to_int(temp_buf[10]);
		buf[4] =
		    char_to_int(temp_buf[11]) << 4 | char_to_int(temp_buf[12]);
#ifdef GSL_APPLICATION
	}
#endif
	...
	else if ('i' == temp_buf[0] && 'd' == temp_buf[1])	/*write id config*/
	{
		...
		if (1 == path_buf[3]) {
			...
			gsl_write_interface(ddata->client, path_buf[6],
					    &path_buf[10], tmp);
		} else if (2 == path_buf[3]) {
			tmp = ((path_buf[5] << 8) | path_buf[4]);
			tmp1 =
			    ((path_buf[9] << 24) | (path_buf[8] << 16) |
			     (path_buf[7] << 8)
			     | path_buf[6]);
			...
			gsl_write_interface(ddata->client, buf[4],
					    &path_buf[10], tmp);
...
static void gsl_timer_check_func(struct work_struct *work)
{
	...
	for (i = 0; i < 5; i++) {
		...
		if (read_buf[3] != 0x5a || read_buf[2] != 0x5a ||
			...
			printk
			    ("gsl_monitor_worker 0xb0 = {0x%02x%02x%02x%02x};\n",
			     read_buf[3], read_buf[2], read_buf[1],
			     read_buf[0]);
			flag = 1;
		} else {
			flag = 0;
...
static int tpd_ps_operate(void *self, uint32_t command, void *buff_in,
				      ...
				      struct device_attribute *attr, char *buf)
{
	...
	ptr += sprintf(ptr, "%08x:", gsl_cfg_table[gsl_cfg_index].data_id[0]);
#endif
	buf_tmp[0] = 0x3;
	buf_tmp[1] = 0;
	buf_tmp[2] = 0;
...
static void gsl_report_work(void)
{
	...
	if (tmp_buf[0] >= 2 && tmp_buf[0] <= 10)
		gsl_read_interface(ddata->client, 0x88, &tmp_buf[8],
				   (tmp_buf[0] * 4 - 4));
	...
	for (tmp = 0; tmp < (cinfo.finger_num > 10 ? 10 : cinfo.finger_num);
		...
		cinfo.y[tmp] =
		    (tmp_buf[tmp * 4 + 4] | ((tmp_buf[tmp * 4 + 5]) << 8));
		cinfo.x[tmp] =
		    (tmp_buf[tmp * 4 + 6] |
		     ((tmp_buf[tmp * 4 + 7] & 0x0f) << 8));
	...
	int tmp1 = 0;
	cinfo.finger_num =
	    (tmp_buf[3] << 24) | (tmp_buf[2] << 16) | (tmp_buf[1] << 8) |
	    (tmp_buf[0]);
...
static int gsl_DrvWire_idt_tp(struct gsl_ts_data *ts)
{
	...
	for (i = 0; i < 3; i++) {
		...
		print_info
		    ("i = %d count0 the test 0xb4 = {0x%02x%02x%02x%02x}\n", i,
		     buf[3], buf[2], buf[1], buf[0]);
		...
		print_info
		    ("i = %d count1 the test 0xb4 = {0x%02x%02x%02x%02x}\n", i,
		     buf[3], buf[2], buf[1], buf[0]);
 ...
 *****************************************************************************/
static int gsl_detect(struct i2c_client *client, int kind,
		      struct i2c_board_info *info)
{
