...
static void __disable_runtime(struct rq *rq)
{
	...
	for_each_rt_rq(rt_rq, iter, rq) {
		 ...
		 */
#ifdef MTK_DEBUG_CGROUP
		printk_deferred(KERN_EMERG "0. disable_runtime, cpu=%d, rd->span=%lu, rt_rq_addr=%x, rt_rq->rt_runtime=%llu, rt_b->rt_runtime=%llu\n",
	...
	if (!lowest_rq) {
		...
		if (likely(p->prio < rq->curr->prio)) {
			...
			mt_rt_printf("1. push_need_released_task fail %d:%s:%d %d",
				p->pid, p->comm, p->prio, rq->curr->prio);
#if 0
			printk_deferred(KERN_ALERT "[sched] push_need_released_task fail %d:%s:%d %d\n",
				p->pid, p->comm, p->prio, rq->curr->prio);
			WARN_ON(1);
#endif
#endif	
